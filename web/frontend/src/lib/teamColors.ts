// NBA team primary and secondary colors indexed by tricode
...
export function getTeamColor(tricode: string): string {
	return teamColors[tricode]?.primary ?? '#3b82f6';
}

export function getTeamGradient(tricode: string): string {
	const colors = teamColors[tricode];
	if (!colors) return 'linear-gradient(135deg, #3b82f6, #f97316)';
